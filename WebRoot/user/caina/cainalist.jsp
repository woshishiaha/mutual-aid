...
<TABLE cellSpacing=0 cellPadding=0 width="120%" align=center border=1>
	...
	<c:if test="${bean.status1=='未确认'}">
  	<a href="${url2 }delete2.action?id=${bean.id }">确认完成</a>
 	</c:if>
 
 	<c:if test="${bean.status2=='确认完成'}">
 	<c:if test="${bean.status1=='确认完成'}"><c:if test="${bean.pj1==null}">
  	<a href="${url2 }update.action?id=${bean.id }">评价</a>
 	</c:if></c:if></c:if>
