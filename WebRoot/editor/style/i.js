config.FixWidth = "";
config.UploadUrl = "../uploadfile/";
config.InitMode = "EDIT";
config.AutoDetectPaste = "1";
config.BaseUrl = "1";
config.BaseHref = "";
config.AutoRemote = "1";
config.ShowBorder = "0";
config.StateFlag = "1";
config.SBCode = "1";
config.SBEdit = "1";
config.SBText = "1";
config.SBView = "1";
config.SBSize = "1";
config.EnterMode = "1";
config.Skin = "office2003";
config.AutoDetectLanguage = "1";
config.DefaultLanguage = "zh-cn";
config.AllowBrowse = "1"; //网络图版=1
config.AllowImageSize = "100"; //图片上传大小 大于0 显示上传框��ʾ�ϴ���
config.AllowFlashSize = "100"; //flash上传大小 大于0 显示上传框��ʾ�ϴ���
config.AllowMediaSize = "100"; //媒体文件上传大小 大于0 显示上传框��ʾ�ϴ���
config.AllowFileSize = "100"; //文件大小属性 上传 大于0 显示上传框��ʾ�ϴ���
config.AllowRemoteSize = "100";
config.AllowLocalSize = "100";
config.AllowImageExt = "gif|jpg|jpeg|bmp";
config.AllowFlashExt = "swf|flv";
config.AllowMediaExt = "rm|mp3|wav|mid|midi|ra|avi|mpg|mpeg|asf|asx|wma|mov";
config.AllowFileExt = "rar|zip|pdf|doc|xls|ppt|chm|hlp";
config.AllowRemoteExt = "gif|jpg|bmp";
config.AllowLocalExt = "gif|jpg|bmp|wmz|png";
config.AreaCssMode = "0";
config.SLTFlag = "0";
config.SLTMinSize = "300";
config.SLTOkSize = "120";
config.SLTMode = "0";
config.SLTCheckFlag = "0";
config.SYWZFlag = "0";
config.SYTPFlag = "0";
config.FileNameMode = "0";
config.PaginationMode = "1";
config.PaginationKey = "{page}";
config.PaginationAutoFlag = "1";
config.PaginationAutoNum = "1000";
config.SParams = "";
config.SpaceSize = "";
config.L = "ok";
config.ServerExt = "jsp";

config.Toolbars = [
	["TBHandle", "FormatBlock", "FontName", "FontSize", "Cut", "Copy", "Paste", "PasteText", /*"PasteWord",*/ "TBSep", "Delete", "RemoveFormat", "TBSep", "FindReplace", "TBSep", "UnDo", "ReDo", "TBSep", "SelectAll", "UnSelect"],
	["TBHandle", "Bold", "Italic", "UnderLine", "StrikeThrough", "SuperScript", "SubScript", "TBSep", "JustifyLeft", "JustifyCenter", "JustifyRight", "JustifyFull","TBSep", "OrderedList", "UnOrderedList", "Indent", "Outdent", "TBSep", "ForeColor", "BackColor", "BgColor", "BackImage", "TBSep", "Fieldset", "HorizontalRule", "Marquee", "TBSep", "CreateLink", "Unlink", "Map", "Anchor"],
	["TBHandle", "Code", "EQ", "TBSep", "Big", "Small", "TBSep", "Image", "Flash", "Media", "File", "GalleryMenu", "TBSep", "RemoteUpload", "LocalUpload","ImportWord", "ImportExcel", "Capture",  "TBSep", "TableMenu", "FormMenu", "TBSep", "QuickFormat", "TBSep", "Template", "Symbol", "Emot", "Art", "Excel", "PrintBreak", "NowDate", "NowTime", "TBSep", "Quote", "ShowBorders", "TBSep", "Pagination", "PaginationInsert"],
	["TBHandle", "absolutePosition", "zIndexBackward", "zIndexForward", "TBSep", "Iframe", "TBSep", "BR", "Paragraph", "ParagraphAttr", "TBSep", "UpperCase", "LowerCase", "TBSep", "Print", "TBSep", "ZoomMenu", "Maximize", "TBSep", "About"]
];